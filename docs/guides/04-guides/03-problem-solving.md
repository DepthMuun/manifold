# DepthMuun Troubleshooting Guide


**Last Updated:** February 2026

This document provides comprehensive troubleshooting guidance for the DepthMuun Geometric Flow Network implementation. It covers common issues, diagnostic procedures, and solutions for problems that users may encounter during installation, training, and inference.
...
**Solution:** DepthMuun requires Python 3.10 or higher. Check your Python version:
...
**Symptoms:** Import errors when importing `gfn`, or errors about missing torch modules.
...
```python
import gfn
model = gfn.create(preset="base")  
# or
model = gfn.create(vocab_size=len(tokenizer))  # Custom vocabulary
```
...
```python
model = gfn.create(dim=512, heads=8)  # 512 / 8 = 64, valid
# vs
model = gfn.create(dim=512, heads=7)  # Error: 512 not divisible by 7
```
...
for t in range(sequence_length):
    logits, state = model(input_ids[:, t:t+1], return_state=True, state=state)
