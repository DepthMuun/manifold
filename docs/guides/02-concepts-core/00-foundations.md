...
Computing Christoffel symbols from a full metric tensor would be computationally expensive, as the metric is a $d \times d$ object for a $d$-dimensional manifold. DepthMuun uses a low-rank parameterization that reduces computational complexity while maintaining sufficient expressivity:

$$\Gamma(v, x) \approx W \cdot \left[ (U^T v)^2 \odot \sigma(\|U^T v\|_2) \right]$$

Here, $U \in \mathbb{R}^{d \times r}$ and $W \in \mathbb{R}^{d \times r}$ are low-rank matrices with rank $r \ll d$. The quadratic operation $(U^T v)^2$ captures interactions between velocity components, the element-wise multiplication with a saturation function $\sigma = 1.0 / (1.0 + \| U^T v \|)$ provides numerical stability, and the final multiplication by $W$ projects back to the full dimension.

> [!WARNING]
> **Theoretical Limitation**: This decomposition is a designed **mathematical approximation** to reduce $O(d^3)$ complexity to $O(d^2 \cdot r)$.
> It maintains $i,j$ symmetry by construction ($ \Gamma^k_{ij} \approx \Gamma^k_{ji} $) but **does not** guarantee true Christoffel properties, such as Bianchi identities, and it cannot be analytically derived from a valid Riemannian metric tensor $g_{ij}$. It acts as an empirical pseudo-connection specifically optimized for ML gradient stability.
...
DepthMuun uses an implicit-friction conformal symplectic Leapfrog integrator. Standard Leapfrog cannot handle physical drag seamlessly, so an implicit substitution is used where the fractional velocity updates are divided by the resistance $\mu$:

$$v_{t+1/2} = \frac{v_t + \frac{dt}{2} \cdot \left( F_t + F_{\text{ghost}} - \Gamma(x_t, v_t) \right)}{1 + \frac{dt}{2} \cdot \mu_t}$$
$$x_{t+1} = \text{Bnd}(x_t + dt \cdot v_{t+1/2})$$
$$v_{t+1} = \frac{v_{t+1/2} + \frac{dt}{2} \cdot \left( F_{t+1} + F_{\text{ghost}} - \Gamma(x_{t+1}, v_{t+1/2}) \right)}{1 + \frac{dt}{2} \cdot \mu_{t+1}}$$

The characteristic "leapfrog" pattern—half-steps in velocity, full steps in position—is augmented with boundary enforcement $\text{Bnd}(\cdot)$ (for Toroidal spaces), an active Ghost Force ($F_{\text{ghost}}$) representing hysteresis, and crucially, an implicit damping term in the denominator. This produces exact volume-contraction phases associated with physical friction, ensuring stable gradient flow without resorting to dissipative Euler methods.
...
where $p$ is momentum, $F_{\text{conservative}}$ is the conservative (Hamiltonian) force, and $\mu(x, F_{ext}, v)$ is the friction coefficient. The friction coefficient is learned as a function of the current state, input forces, and velocity, allowing the system to autonomously determine when to preserve information and when to update.

In the implementation, the friction coefficient is computed with velocity-dependent drag:

```python
gate_activ = torch.matmul(W_forget, x) + torch.matmul(W_input, F_ext) + b
mu_base = torch.sigmoid(gate_activ) * 5.0
mu_final = mu_base * (1.0 + alpha * ||v||)
```

The sigmoid activation ensures that the friction coefficient is always positive and bounded within the scale factor (`[0, 5.0]`).
Crucially, the higher the particle's velocity, the stronger the drag it experiences ($\alpha \|v\|$). This mirrors physical air resistance and automatically damps out uncontrolled explosive trajectories common in initial training phases.
...
The functional embedding breaks down large integer token identifiers into their structural components (binary digits), scaling log-linearly rather than linearly:

```python
# Convert token_id to a boolean bit-array
mask = 2 ** torch.arange(coord_dim)
bits = (token_id & mask) > 0

# Binary Mode: Map {0, 1} -> {-1, 1} and feed into the SIREN stack
coords_siren = bits.float() * 2 - 1 

# Linear Mode: Treat bits directly as force channels {0, 1} for pure functional parity
coords_linear = bits.float()
```

- **Linear Mode**: Rather than using a SIREN network, this mode acts as an identity pass-through. It uses the binary sequence `[1, 0, 1...]` as immediate force triggers, making it optimal for arithmetic tasks, algorithmic parity, and direct boolean masking.
- **Binary Mode**: Discretizes the bits into $\{-1, 1\}$ bounds and feeds them through the SIREN network for complex feature generation and hierarchical hash expansion, ideal for language contexts where adjacent words may have unrelated IDs.
...
The Christoffel symbols are bounded asymptotically using a `tanh` projection to prevent numerical instability around curvature singularities, while preserving differentiability:

```python
Gamma = 5.0 * torch.tanh(Gamma / 5.0)
```

This asymptotic scaling prevents the curvature from becoming arbitrarily large (which could cause numerical overflow or division by near-zero values), ensuring that backpropagation continuously receives smooth gradients even at the edge of the stability region.
