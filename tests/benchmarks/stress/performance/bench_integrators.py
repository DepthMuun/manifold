...
def measure_drift(model, drift_steps=100, device='cuda'):
    ...
    with torch.no_grad():
        ...
        tx, tv = integrator(x, v, force=None, steps=drift_steps, 
                          W_forget_stack=W_f, W_input_stack=W_i, b_forget_stack=b_f if 'None, steps=drift_steps, 
                          W_forget_stack=W_f, W_input_stack=W_i, b_forget_stack=b_f' != '' else None, dt=dt, geometry=harmonic_geo if 'harmonic_geo' in locals() else flat_geo_cpu)
...
def run_integrator_suite():
    logger = ResultsLogger("integrators", category="performance/runtime")
