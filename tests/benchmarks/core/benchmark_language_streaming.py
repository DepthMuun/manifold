    ...
    if isinstance(output, tuple) and len(output) >= 6:
        ...
        if christoffels:
            ...
            loss_ham = hamiltonian_loss(
                v_seq, states=x_seq, metric_fn=first_head_metric, lambda_h=1e-4, forces=all_forces
            )
    return loss_val + loss_phy + loss_ham


def run_training(model, loader, optimizer, scheduler, device, max_steps, log_every, pad_token_id, vocab_size):
    model.train()
    total_loss = 0.0
    ...
    for step in pbar:
        ...
        if step % log_every == 0 or step == 1:
            pbar.set_postfix(loss=f"{avg:.4f}", acc=f"{acc*100:.2f}%")
    return total_loss / max_steps


@torch.no_grad()
def run_evaluation(model, loader, device, max_steps, pad_token_id, vocab_size):
    model.eval()
    total_loss = 0.0
    data_iter = iter(loader)
...
def run_language_streaming_test(args):
    ...
    # 1. Main weights (Euclidean, with decay)
    main_params = [
        p for n, p in model.named_parameters()
        if not any(x in n for x in ["x0", "v0", "impulse_scale", "gate"])
    ]
    
    # 2. Toroidal parameters (x0 - Initial State) -> Use 'torus' retraction
    toroidal_params = [
        p for n, p in model.named_parameters()
        if "x0" in n
    ]
    
    # 3. Sensitive Euclidean parameters (v0, gates, scales) -> Use 'euclidean', no decay
    sensitive_params = [
        p for n, p in model.named_parameters()
        if any(x in n for x in ["v0", "impulse_scale", "gate"]) and "x0" not in n
    ]
    ...
    try:
        scheduler = optim.lr_scheduler.OneCycleLR(
            optimizer, max_lr=2e-3, total_steps=args.steps, pct_start=0.2
        )
    except Exception:
        scheduler = None
    ...
    t0 = time.time()
    print(f"Modelo construido: {vocab_size} vocab, {args.dim} dim, {args.depth} layers, {args.heads} heads")
    ...
    print(f"Eval Loss: {eval_loss:.4f} | Reload Eval Loss: {reload_eval_loss:.4f}")
